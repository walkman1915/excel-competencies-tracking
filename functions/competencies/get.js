...
exports.lambdaHandler = async (event, context) => {
    try {
        ...
        const allCompetencies = await getCompetencies(params);

        // Generate the response body for a successful get
        let respBody = {};
        respBody.Items = allCompetencies.Items; //Gets the actual items from the call
        ...
        if ("LastEvaluatedKey" in allCompetencies) {
            respBody.LastEvaluatedKey = Buffer.from(JSON.stringify(allCompetencies.LastEvaluatedKey),'binary').toString('base64');
        }
 ...
 */
function getCompetencies(params) {
